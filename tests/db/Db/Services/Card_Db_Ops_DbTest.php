...
namespace Mtgtools\Tests\db\Db\Services;
...
class Card_Db_Ops_DbTest extends Mtgtools_UnitTestCase
{
    ...
    public function testNewlyInsertedImageHasCorrectAttributes( array $row ) : void
    {
        ...
        $this->assertLessThanOrEqual(new DateTime('now'), new DateTime($row['cached']), 'Failed to assert that the timestamp on a newly cached image uri is equal to or older than current time.' );
    }
